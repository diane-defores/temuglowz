...
- Backend sync pagination for `syncRecords` reads in `convex/sync.ts`.
- Public action and internal query response shape updates for cursor/continuation metadata.
- Frontend action wrapper updates in `src/lib/cloudSyncBackend.ts`.
- Backend-verified sync handoff loop updates in `src/lib/cloudSync.ts`.
...
- Design-system changes must respect `shipflow_data/technical/design-system-authority.md`; do not introduce new one-off visual literals for app routes.
...
- `pnpm build`
- `pnpm build:site`
- `python3 /home/claude/shipflow/tools/design_system_drift_check.py --changed --format markdown` when guide/card/layout styling files change
...
- Android device proof is not required unless implementation changes `src-tauri/`, mobile app routes, or native sync behavior. Use `exception-with-proof`: this spec changes web/static and Convex/client sync logic only.
...
- Existing sync entitlement contract: `shipflow_data/workflow/specs/temu-shopping-lists-premium-cloud-sync.md`.
- Existing app technical context: `shipflow_data/technical/apps/temu-shopping-lists-android-app.md`.
...
- `convex/sync.ts`: backend contract changes affect generated API behavior and tests.
- `src/lib/cloudSyncBackend.ts`: action response/request types must match Convex response shape.
- `src/lib/cloudSync.ts`: post-auth handoff must wait for all pages before queued push replay.
- `src/lib/cloudSyncHydration.ts`: record application must remain idempotent under page loops and duplicate/stale records.
- `src/pages/guides/*.astro`: product card rendering changes affect static HTML and SEO.
- `src/site/components/ProductCard.vue`: may remain for genuinely interactive contexts, but guide pages should not hydrate it unnecessarily.
- `src/layouts/Layout.astro`: Lenis changes affect every static page and anchor/reveal behavior.
- `public/` and `site/public/`: asset consolidation can affect deployed URLs; public URL compatibility must be checked before deleting files.
...
- Update `shipflow_data/technical/apps/temu-shopping-lists-android-app.md` if the cloud sync hydration contract changes materially, especially cursor semantics or validation commands.
...
- Do not edit `TASKS.md` or `AUDIT_LOG.md` from this spec; tracking was already created by `403-sf-perf`.
