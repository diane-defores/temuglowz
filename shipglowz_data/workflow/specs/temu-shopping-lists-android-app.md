...
linked_systems:
  ...
  - "Temu product links"
depends_on:
  - artifact: "shipglowz_data/workflow/research/2026-06-09-temu-shopping-lists-android.md"
    artifact_version: "1.0.0"
...
Checklist path: `shipglowz_data/workflow/test-checklists/temu-shopping-lists-android.md`.
...
- `shipglowz_data/technical/architecture.md` after implementation begins, if project governance is initialized.
- `shipglowz_data/workflow/test-checklists/temu-shopping-lists-android.md` for real-device manual QA.
...
- [ ] Task 1: Initialize the app scaffold from Socialglowz patterns, not by wholesale copying unrelated domain code.
  - File: `app/package.json`, `app/vite.config.ts`, `app/tsconfig*.json`, `app/src/`, `app/src-tauri/`
...
- [ ] Task 2: Define product/list domain types and validators.
  - File: `app/src/domain/temu/types.ts`, `app/src/domain/temu/validators.ts`, `app/src/domain/temu/url.ts`, `app/src/domain/temu/importParser.ts`
...
- [ ] Task 3: Build local-first Pinia stores.
  - File: `app/src/stores/shoppingLists.ts`, `app/src/stores/productSnapshots.ts`, `app/src/stores/importDrafts.ts`, `app/src/utils/pinia.ts`
...
- [ ] Task 4: Implement Android share-target intake.
  - File: `app/src-tauri/`, `app/src-tauri/gen/android/app/src/main/AndroidManifest.xml` or Tauri Android config equivalents, `app/src-tauri/plugins/share-intent/`, `app/src/platform/shareIntent.ts`
...
- [ ] Task 5: Build import review and manual URL screens.
  - File: `app/src/views/ImportReviewView.vue`, `app/src/views/ManualImportView.vue`, `app/src/components/import/*`, `app/src/router/*`
...
- [ ] Task 6: Build shopping list and product detail UI.
  - File: `app/src/views/ListsView.vue`, `app/src/views/ListDetailView.vue`, `app/src/views/ProductDetailView.vue`, `app/src/components/products/*`
...
- [ ] Task 7: Add image cache and snapshot storage policy.
  - File: `app/src/domain/temu/images.ts`, `app/src/stores/productSnapshots.ts`, `app/src-tauri/src/` or web/local storage adapter
...
- [ ] Task 8: Add backup/export.
  - File: `app/src/features/backup/*`, `app/src-tauri/src/backup.rs`, `app/src-tauri/plugins/share-intent/android/*` if native file save is needed
...
- [ ] Task 9: Add optional Convex schema and sync foundation.
  - File: `app/convex/schema.ts`, `app/convex/shoppingLists.ts`, `app/convex/productSnapshots.ts`, `app/src/lib/cloudSync.ts`, `app/src/lib/cloudSyncQueue.ts`
...
- [ ] Task 10: Write real-device QA checklist and README.
  - File: `README.md`, `shipglowz_data/workflow/test-checklists/temu-shopping-lists-android.md`
...
- [ ] Task 11: Cart import investigation spike.
  - File: `shipglowz_data/workflow/research/temu-cart-share-payloads.md` or `docs/explorations/temu-cart-import.md`
...
- `shipglowz_data/workflow/research/2026-06-09-temu-shopping-lists-android.md`
...
| 2026-06-09 22:03:58 UTC | sf-docs | GPT-5 Codex | Bootstrapped `shipglowz_data/technical` and mapped planned code coverage for Governance Corpus Gate | Technical docs baseline added for clean-slate scaffold; audit-ready map + temp coverage status | /sf-start Temu shopping lists Android app |
