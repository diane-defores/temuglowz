...
updated: "2026-06-12"
updated_at: "2026-06-12 03:51:30 UTC"
status: ready
...
linked_systems:
  ...
  - "app/src/lib/convexAuth.ts"
  - "app/src/lib/postAuthSyncFeedback.ts"
  - "app/src/lib/syncMerge.ts"
  - "app/src/types/sync.ts"
  - "app/src/lib/accessModel.ts"
  - "app/src/components/PostAuthSyncFeedback.vue"
...
depends_on:
  ...
  - artifact: "/home/claude/shipglowz/skills/600-sf-local-cloud-sync/references/local-cloud-sync-doctrine.md"
    artifact_version: "1.0.0"
    required_status: draft
  - artifact: "/home/claude/shipglowz/skills/600-sf-local-cloud-sync/references/ux-security-checklist.md"
    artifact_version: "1.0.0"
    required_status: draft
...
next_step: "/405-sf-prod Confirm Android/hosted proof target for premium cloud sync remediation, then /109-sf-auth-debug authenticated sync smoke and /107-sf-test Android native auth-cycle proof"
---
...
Checklist path: `shipglowz_data/workflow/test-checklists/temu-shopping-lists-premium-cloud-sync.md`.
...
- Entitlements spec: `shipglowz_data/workflow/specs/temu-shopping-lists-entitlements-access-model.md`, version `1.0.5`, reviewed. This cloud sync spec depends on its suite-ledger and fail-closed access model.
- Android app spec: `shipglowz_data/workflow/specs/temu-shopping-lists-android-app.md`, version `1.0.0`, ready.
- Technical app context: `shipglowz_data/technical/apps/temu-shopping-lists-android-app.md`, version `1.0.2`, draft.
- Local-cloud sync doctrine: `/home/claude/shipglowz/skills/600-sf-local-cloud-sync/references/local-cloud-sync-doctrine.md`, version `1.0.0`, draft.
- Sync UX/security checklist: `/home/claude/shipglowz/skills/600-sf-local-cloud-sync/references/ux-security-checklist.md`, version `1.0.0`, draft.
...
- `app/src/types/domain.ts`: may need sync metadata types, versioned payload types, and explicit cloud-safe payload boundaries.
- `app/src/stores/shoppingLists.ts`: list and item mutations must enqueue typed sync operations when sync is enabled.
- `app/src/stores/productSnapshots.ts`: snapshot writes/deletes must enqueue typed sync operations and keep URL-only media behavior clear.
- `app/src/lib/cloudSyncQueue.ts`: current unknown-payload local queue must become a typed durable operation queue with idempotency, account marker, retry metadata, and corruption handling.
- `app/src/lib/cloudSync.ts`: current placeholder must become a sync state machine, not only a boolean flag.
- `app/src/lib/convexAuth.ts`, `app/convex/auth.ts`, `app/convex/http.ts`: SocialGlowz-style identity plumbing exists and must stay separated from product access. Future work must not treat `isAuthenticated` as premium authorization.
- `app/src/lib/postAuthSyncFeedback.ts`, `app/src/components/PostAuthSyncFeedback.vue`: post-auth feedback exists and must report only identity/preparation until remote sync writes/hydration are proven.
- `app/src/lib/accessModel.ts`: existing protected feature `cloud_sync` remains the local/UI contract; backend must recompute access server-side.
- `app/convex/schema.ts`: current `userId` scaffold must be replaced or adapted to server-owned account/owner ids, sync metadata, tombstones, indexes, and environment scoping.
- New Convex modules likely needed: `app/convex/sync.ts`, `app/convex/syncAccess.ts`, `app/convex/users.ts`, and generated API references after Convex codegen.
...
- `shipglowz_data/technical/apps/temu-shopping-lists-android-app.md`: document cloud sync architecture, account boundary, sync data domains, and proof limits.
- `shipglowz_data/workflow/test-checklists/temu-shopping-lists-premium-cloud-sync.md`: add the required TC-SYNC scenarios.
- `shipglowz_data/technical/support/entitlements-runbook.md`: add support triage for premium sync blocked states, account mismatch, and entitlement loss once sync ships.
...
- [ ] Task 1: Define sync domain and metadata types.
  - Files: `app/src/types/domain.ts`, new `app/src/types/sync.ts`.
...
- [ ] Task 2: Build deterministic merge/conflict/tombstone logic.
  - Files: new `app/src/lib/syncMerge.ts`, new `app/src/lib/syncMerge.test.ts`.
...
- [ ] Task 3: Replace unknown local queue with typed durable operations.
  - Files: `app/src/lib/cloudSyncQueue.ts`, new `app/src/lib/cloudSyncQueue.test.ts`.
...
- [ ] Task 4: Add sync state machine and UI-facing status model.
  - Files: `app/src/lib/cloudSync.ts`, new `app/src/stores/cloudSync.ts`, route/component files as needed.
...
- [ ] Task 5: Adapt local stores to enqueue sync operations without weakening local-first behavior.
  - Files: `app/src/stores/shoppingLists.ts`, `app/src/stores/productSnapshots.ts`, related tests.
...
- [ ] Task 7: Implement Convex access and identity bridge contract.
  - Files: new `app/convex/syncAccess.ts`, `app/convex/users.ts`, possible suite bridge module.
...
- [ ] Task 7A: Replace post-auth preparation placeholder with real entitlement-aware sync handoff.
  - Files: `app/src/lib/cloudSync.ts`, `app/src/lib/postAuthSyncFeedback.ts`, `app/src/components/PostAuthSyncFeedback.vue`, `app/src/app-pages/SyncPage.vue`, `app/src/ui/temu-shell/components/MobileSettingsSheet.vue`.
...
- [ ] Task 8: Implement authorized cloud sync functions.
  - Files: new `app/convex/sync.ts` and generated API updates.
...
- [ ] Task 9: Connect client sync service to backend functions.
  - Files: `app/src/lib/cloudSync.ts`, `app/src/lib/cloudSyncQueue.ts`, new backend API adapter files.
...
- [ ] Task 10: Add user-facing sync settings and conflict/retry UX.
  - Files: route/component files under `app/src/app-pages/`, `app/src/router/index.ts`, `app/src/App.vue`, `app/src/styles.css` as needed.
...
- [ ] Task 11: Write sync checklist and docs.
  - Files: `shipglowz_data/workflow/test-checklists/temu-shopping-lists-premium-cloud-sync.md`, `README.md`, `shipglowz_data/technical/apps/temu-shopping-lists-android-app.md`.
...
- Existing `app/convex/schema.ts` is not safe for production sync because its `userId` fields are client-shape scaffolding. Implementation must make the server derive ownership.
...
| 2026-06-10 17:29:49 UTC | continue | GPT-5 Codex | Created the Convex cloud project, generated `app/convex/_generated`, and ran deployed fail-closed proof | Partial: Convex project and dev deployment exist, codegen works, deployed unauthenticated status query returns `missing_identity`; suite entitlement bridge, authenticated client wiring, real writes, hydration/promotion, and device proof remain | /sf-start Continue premium cloud sync entitlement bridge and authenticated client wiring |
...
| 2026-06-12 03:44:00 UTC | 001-sf-build | GPT-5 Codex + gpt-5.3-codex-spark workers | Implemented the remediation slice for the 2026-06-12 code audit: live Convex sync mutations now reject stale writes, hydration skips stale remote records and tombstones, and Tauri auth token persistence moved behind native app commands with non-localStorage fallback | Implemented locally with tests/build proof: targeted sync/auth tests, `pnpm typecheck`, `pnpm lint:check`, and `pnpm build` pass; remaining proof is authenticated hosted sync plus Tauri/Android auth-cycle validation, and host `cargo check` is still blocked here by missing `pkg-config`/GTK tooling | /103-sf-verify Verify premium cloud sync stale-write/tombstone fixes and native auth token storage path |
| 2026-06-12 03:49:14 UTC | 103-sf-verify | GPT-5 Codex | Verified the stale-write/tombstone remediation and Tauri-native auth token storage slice against focused tests, checklist status, metadata, docs freshness, runtime diagnostics surface, and local build gates | Partial: local sync/auth tests, `pnpm typecheck`, `pnpm lint:check`, `pnpm build`, metadata lint, and diagnostics/header surface pass; required checklist rows for authenticated promotion/hydration/device recovery remain `NOT_RUN`, and host `cargo check` is environment-blocked here by missing `pkg-config`/GTK tooling rather than the new auth-storage code | /005-sf-ship Push premium cloud sync remediation, then /405-sf-prod confirm Android/hosted proof target, then /109-sf-auth-debug authenticated sync smoke and /107-sf-test Android native auth-cycle proof |
| 2026-06-12 03:51:30 UTC | 005-sf-ship | GPT-5 Codex | Quick-shipped the chantier trace after `103-sf-verify` by committing and pushing the updated premium cloud sync spec only; the verified remediation code was already present on `preview`/`origin/preview` | Shipped: `103-sf-verify` trace is now pushed on `preview`; no task/changelog closeout was attempted, and hosted/authenticated sync proof plus Android native auth-cycle proof remain pending | /405-sf-prod Confirm Android/hosted proof target for premium cloud sync remediation, then /109-sf-auth-debug authenticated sync smoke and /107-sf-test Android native auth-cycle proof |
...
- sf-verify: partial on 2026-06-12; local unit/build proof, metadata lint, docs freshness, and runtime diagnostics/header checks pass for the remediation slice, but required authenticated promotion/hydration/device proof is still pending and host `cargo check` remains environment-blocked here because `pkg-config`/GTK tooling is missing
- sf-ship: previous bridge/deploy slice remains pushed (`14057d3` for Temu; `932a3f6`, `1b4d5d5`, and `e5128bd` for WinFlowz), and the 2026-06-12 `103-sf-verify` chantier trace is now quick-shipped on `preview`
- sf-prod: WinFlowz Convex prod and Vercel prod deployed; Temu Convex dev bridge env configured
- sf-end: not launched

Next command: `/405-sf-prod Confirm Android/hosted proof target for premium cloud sync remediation, then /109-sf-auth-debug authenticated sync smoke and /107-sf-test Android native auth-cycle proof`
