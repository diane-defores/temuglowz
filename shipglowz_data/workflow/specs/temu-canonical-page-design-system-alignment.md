...
linked_systems:
  ...
  - "app/src/app-pages/ManualImportPage.vue"
  - "app/src/app-pages/SyncPage.vue"
  - "shipglowz_data/technical/design-system-authority.md"
depends_on:
  - artifact: "shipglowz_data/workflow/specs/temu-socialglow-ui-copy-migration.md"
    artifact_version: "1.0.2"
    required_status: ready
  - artifact: "shipglowz_data/workflow/specs/temu-home-shopping-lists-redesign.md"
...
evidence:
  ...
  - "503-sf-audit-design-tokens found app/src/styles.css legacy classes with hardcoded blue/gray scaffold values."
...
The app's Settings page already reflects the intended SocialGlow-inspired design system, but list detail, manual import, and sync routes are still standalone legacy pages. They use `app/src/styles.css` scaffold classes such as `.panel`, `.item-card`, `.notice`, `.sync-onboarding-card`, and global blue buttons. This creates a visible product break exactly when the user opens the core object of the app: a shopping list.

## Solution

Declare `temu-shell` as the design-system authority, extend `app/src/ui/temu-shell/style.css` with reusable page primitives, and migrate the three routes to those primitives. Keep business logic intact while replacing legacy classes and inline visual styles with shared canonical classes and CSS variables.

## Scope In

- Add `shipglowz_data/technical/design-system-authority.md`.
- Extend `app/src/ui/temu-shell/style.css` with page shell, content cards, form fields, status pills, action groups, empty states, and item/list detail primitives using existing or newly named design-system variables.
- Ensure `app/src/ui/temu-shell/style.css` is imported globally enough for standalone routes.
- Migrate `app/src/app-pages/ListDetailPage.vue` to canonical page primitives.
- Migrate `app/src/app-pages/ManualImportPage.vue` to canonical page primitives.
- Migrate `app/src/app-pages/SyncPage.vue` to canonical page primitives.
...
- The canonical visual source is `app/src/ui/temu-shell/style.css`, with Settings as component reference.
...
Required results:
- `python3 /home/claude/shipglowz/tools/design_system_drift_check.py --changed --format markdown` reports no unapproved drift for changed UI files.
...
- Route screenshots:
  - `shipglowz_data/workflow/verification/temu-canonical-page-design-system-alignment/list-detail-desktop.png`
  - `shipglowz_data/workflow/verification/temu-canonical-page-design-system-alignment/list-detail-mobile.png`
  - `shipglowz_data/workflow/verification/temu-canonical-page-design-system-alignment/manual-import-desktop.png`
  - `shipglowz_data/workflow/verification/temu-canonical-page-design-system-alignment/manual-import-mobile.png`
  - `shipglowz_data/workflow/verification/temu-canonical-page-design-system-alignment/sync-desktop.png`
  - `shipglowz_data/workflow/verification/temu-canonical-page-design-system-alignment/sync-mobile.png`
...
- `app/src/App.vue` root and router outlet.
- `app/src/router/index.ts` standalone page routes.
- `app/src/ui/temu-shell/style.css` existing canonical tokens and Settings classes.
- `app/src/ui/temu-shell/components/MobileSettingsSheet.vue` canonical Settings reference.
- `app/src/stores/shoppingLists.ts`, `app/src/stores/productSnapshots.ts`, and `app/src/stores/productObservations.ts` existing list/detail data.
- `app/src/lib/cloudSync.ts`, `app/src/lib/cloudSyncQueue.ts`, and `app/src/lib/convexAuth.ts` existing sync page behavior.
...
- `app/src/main.ts` currently imports `app/src/styles.css` globally; standalone migrated routes need access to canonical `temu-shell` styles as well.
- `app/src/styles.css` can remain for notification and compatibility styles, but migrated route templates should no longer rely on `.panel`, `.item-card`, `.notice`, `.sync-onboarding-card`, `.link-btn`, or global blue `button` styles.
- `shipglowz_data/technical/code-docs-map.md` already maps `app/src/**` UI/state changes to the app technical doc.
- The task created by audit 503 should be marked done after verified implementation.

## Documentation Coherence

Update `shipglowz_data/technical/apps/temu-shopping-lists-android-app.md` with the design-system authority and migrated route status. README changes are not required because no public setup, product promise, billing, or Android behavior changes.
...
- [ ] Task 1: Wire the canonical style layer globally.
  - Files: `app/src/main.ts`, `app/src/ui/temu-shell/style.css`
...
- [ ] Task 3: Migrate shopping list detail.
  - File: `app/src/app-pages/ListDetailPage.vue`
...
- [ ] Task 4: Migrate manual import.
  - File: `app/src/app-pages/ManualImportPage.vue`
...
- [ ] Task 5: Migrate synchronization.
  - File: `app/src/app-pages/SyncPage.vue`
...
- [ ] Task 6: Update docs and tracker.
  - Files: `shipglowz_data/technical/apps/temu-shopping-lists-android-app.md`, `shipglowz_data/workflow/TASKS.md`, current spec.
...
- Migrated page templates consume shared canonical classes from `app/src/ui/temu-shell/style.css`.
...
- Medium visual regression risk if global `app/src/styles.css` button rules override canonical buttons. Mitigation: use specific canonical classes and import order.
- Medium direct-route risk if `temu-shell` CSS was previously only imported by the shell component. Mitigation: import canonical CSS in `app/src/main.ts`.
...
- Low docs drift risk if the new design-system authority is not recorded. Mitigation: create/update `shipglowz_data/technical/design-system-authority.md`.
...
- Put new visual primitives in `app/src/ui/temu-shell/style.css`.
...
- `app/src/main.ts` imports the canonical style layer globally and applies `html.dark` from the persisted sessions setting, so standalone routes do not need to visit `/` first.
- `app/src/ui/temu-shell/style.css` now owns canonical page primitives used by migrated routes.
- `app/src/app-pages/ListDetailPage.vue`, `app/src/app-pages/ManualImportPage.vue`, and `app/src/app-pages/SyncPage.vue` no longer use `.panel` or inline visual style attributes.
- Design-system drift scan still reports existing and new candidates in `app/src/ui/temu-shell/style.css` because the current checker flags the declared design-system authority file itself. The migrated page files passed the targeted local-literal scan.
