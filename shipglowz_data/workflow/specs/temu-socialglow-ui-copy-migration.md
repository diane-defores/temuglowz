...
- Preserve Temu-specific code:
  ...
  - `app/src/stores/importDrafts.ts`
  - `app/src/app-pages/ImportReviewPage.vue`
...
- `shipglowz_data/workflow/test-checklists/temu-socialglow-ui-copy-migration.md`
...
- The copied SocialGlow-derived shell is mounted by `app/src/App.vue` or by a direct replacement imported from `app/src/ui/temu-shell/App.vue`.
- The first screen is a shopping-session launcher, not `app/src/app-pages/ShoppingDashboardPage.vue`.
- The session launcher reads from `app/src/stores/shoppingSessions.ts` and writes real Temu shopping sessions.
- `app/src-tauri/plugins/android-temu-webview/android/src/main/java/com/temushoppinglists/temuwebview/TemuWebViewPlugin.kt` exposes the SocialGlow-style quick menu without visible profile entries.
- The native bottom-bar switcher receives session summaries from `app/src/lib/temuWebview.ts` and renders session names.
- Capture still flows through `app/src/stores/importDrafts.ts` into `app/src/app-pages/ImportReviewPage.vue`.
...
- Existing Temu WebView spec: `shipglowz_data/workflow/specs/temu-shopping-webview-sessions.md`.
- Existing Android app spec: `shipglowz_data/workflow/specs/temu-shopping-lists-android-app.md`.
...
- `shipglowz_data/technical/apps/temu-shopping-lists-android-app.md`: record that UI is copied/adapted from SocialGlow and profiles are hidden.
- `shipglowz_data/technical/platforms/android.md`: record native bottom-bar menu behavior and Blacksmith proof.
- `shipglowz_data/workflow/test-checklists/temu-socialglow-ui-copy-migration.md`: add manual Android scenarios.
...
1. Baseline copy inventory:
   - Target files: implementation report plus copied target tree under `app/src/ui/temu-shell/` or direct replacements in `app/src/App.vue` and `app/src/components/`.
...
2. Copy SocialGlow shell:
   - Target files: `app/src/App.vue`, `app/src/ui/temu-shell/App.vue`, `app/src/ui/temu-shell/components/MobileLayout.vue`, `app/src/ui/temu-shell/components/MobileSettingsSheet.vue`, `app/src/ui/temu-shell/components/NetworkWebviewHost.vue`, `app/src/ui/temu-shell/components/AppHeader.vue`, `app/src/ui/temu-shell/components/AppSidebar.vue`, `app/src/ui/temu-shell/components/AppRightSidebar.vue`, `app/src/ui/temu-shell/style.css`, `app/src/ui/temu-shell/assets/main.css`, `app/src/ui/temu-shell/utils/textZoom.ts`, and optionally `app/src/ui/temu-shell/utils/tapSound.ts`.
   - Action: copy SocialGlow shell files into Temu, then wire `app/src/App.vue` to the copied shell or replace it directly.
...
3. Replace social networks with shopping sessions:
   - Target files: `app/src/ui/temu-shell/components/MobileLayout.vue`, `app/src/stores/shoppingSessions.ts`, and any copied helper/store introduced from SocialGlow.
...
4. Remove visible profiles:
   - Target files: `app/src/ui/temu-shell/components/MobileLayout.vue`, `app/src/ui/temu-shell/components/MobileSettingsSheet.vue`, copied onboarding/settings components, and native quick menu code.
...
5. Adapt WebView host:
   - Target files: `app/src/ui/temu-shell/components/NetworkWebviewHost.vue`, `app/src/lib/temuWebview.ts`, `app/src/stores/shoppingSessions.ts`.
   - Action: replace SocialGlow WebView store/composable calls with `app/src/lib/temuWebview.ts` and `app/src/stores/shoppingSessions.ts`.
...
6. Adapt native bottom bar:
   - Target file: `app/src-tauri/plugins/android-temu-webview/android/src/main/java/com/temushoppinglists/temuwebview/TemuWebViewPlugin.kt`.
...
7. Preserve Temu app features:
   - Target files: `app/src/router/index.ts`, `app/src/app-pages/ListsPage.vue`, `app/src/app-pages/ListDetailPage.vue`, `app/src/app-pages/ManualImportPage.vue`, `app/src/app-pages/ImportReviewPage.vue`, `app/src/app-pages/SyncPage.vue`, `app/src/app-pages/ProductDetailPage.vue`.
...
8. Delete rough scaffold:
   - Target files: `app/src/app-pages/ShoppingDashboardPage.vue`, `app/src/router/index.ts`, `app/src/App.vue`.
...
9. Add/adjust tests:
   - Target files: `app/src/stores/shoppingSessions.test.ts`, `app/src/lib/temuWebview.test.ts`, and new UI-shell tests if practical.
...
10. Update docs and checklist:
    - Target files: `README.md`, `shipglowz_data/technical/apps/temu-shopping-lists-android-app.md`, `shipglowz_data/technical/platforms/android.md`, `shipglowz_data/workflow/test-checklists/temu-socialglow-ui-copy-migration.md`.
...
- `shipglowz_data/workflow/specs/temu-socialglow-ui-copy-migration.md`
...
- copied styles/assets/utilities under `app/src/ui/temu-shell/`
...
- Copy the SocialGlow shell into `app/src/ui/temu-shell/`.
- Adapt visible language from social networks to shopping sessions.
- Remove visible profile UI from the copied shell.
- Do not edit `app/src/App.vue`, `app/src/router/index.ts`, or native Kotlin in this batch.
...
- `app/src-tauri/plugins/android-temu-webview/android/src/main/java/com/temushoppinglists/temuwebview/TemuWebViewPlugin.kt`
...
- `app/src/stores/shoppingSessions.ts`
- `app/src/stores/shoppingSessions.test.ts`
- `app/src/lib/temuWebview.ts`
...
- final integration edits inside `app/src/ui/temu-shell/**`
...
- `app/src/App.vue` and `app/src/router/index.ts` should have one owner: Batch D.
- Native plugin Kotlin should have one owner: Batch B.
- `app/src/ui/temu-shell/**` should have one primary owner until Batch D integration.
...
- Read first: `/home/claude/socialglowz/src/ui/setup/pages/SocialGlowz/App.vue`, `MobileLayout.vue`, `MobileSettingsSheet.vue`, `NetworkWebviewHost.vue`, `/home/claude/socialglowz/src-tauri/plugins/android-webview/android/src/main/java/com/socialglowz/webview/NativeWebViewPlugin.kt`, `app/src/stores/shoppingSessions.ts`, `app/src/lib/temuWebview.ts`, `app/src/router/index.ts`, and `app/src/App.vue`.
