...
Checklist path: `shipglowz_data/workflow/test-checklists/temu-shopping-webview-sessions.md`.
...
- Existing product snapshot/import flow in `app/src/stores/productSnapshots.ts`, `app/src/stores/shoppingLists.ts`, `app/src/app-pages/ImportReviewPage.vue`, and `app/src/lib/importParser.ts`.
...
- `shipglowz_data/workflow/test-checklists/temu-shopping-webview-sessions.md`: real-device manual proof steps.
...
1. Copy and rename the SocialGlowz plugin scaffold into `app/src-tauri/plugins/android-temu-webview`.
...
3. Register the plugin in `app/src-tauri/Cargo.toml`, `app/src-tauri/src/lib.rs`, and Android Gradle/Tauri plugin files.
...
4. Add shopping session domain types.
   - Update `app/src/types/domain.ts` with `ShoppingSession`, `ShoppingSessionSettings`, and `WebviewCaptureResult`.
   - Add `source: "webview"` to import/snapshot source types where required.

5. Add Pinia stores and tests.
   - Create `app/src/stores/shoppingSessions.ts`.
...
6. Add a WebView bridge module.
   - Create `app/src/lib/temuWebview.ts`.
...
7. Build the Shopping dashboard UI.
   - Create `app/src/app-pages/ShoppingDashboardPage.vue`.
   - Add route in `app/src/router/index.ts` and navigation entry in `app/src/App.vue`.
...
- Android Tauri plugin crate `app/src-tauri/plugins/android-temu-webview` with Temu-only WebView hosts, native bottom bar, safe URL allowlist, dark mode, text zoom, capture event, and degraded-mode fallback.
...
- README WebView section and `shipglowz_data/workflow/test-checklists/temu-shopping-webview-sessions.md`.
