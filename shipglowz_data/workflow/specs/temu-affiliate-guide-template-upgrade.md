...
linked_systems:
  ...
  - "site/public/data/kitchen-gadgets.json"
...
🟡 [temu] spec: Temu affiliate guide template upgrade | status: reviewed | path: shipglowz_data/workflow/specs/temu-affiliate-guide-template-upgrade.md | next: /100-sf-spec Temu affiliate guide template upgrade
...
- Optional update to `site/public/llms.txt` if it currently implies stronger verification than the site can support.
...
- `python3 /home/claude/shipglowz/tools/design_system_drift_check.py --changed --format markdown` reports no unapproved drift for touched UI/template files.
- `python3 /home/claude/shipglowz/tools/shipflow_metadata_lint.py shipglowz_data/workflow/specs/temu-affiliate-guide-template-upgrade.md shipglowz_data/technical/design-system-authority.md` passes.
...
- `python3 /home/claude/shipglowz/tools/shipflow_metadata_lint.py shipglowz_data/workflow/specs/temu-affiliate-guide-template-upgrade.md shipglowz_data/technical/design-system-authority.md`
- focused `rg` or equivalent static scans for unsupported public-claim terms in `site/src/site/data`, guide routes/components, `site/public/llms.txt` if touched, and generated `dist-site/guides/**/index.html`
...
- Current data files: `site/src/site/data/kitchen-gadgets.json`, `site/src/site/data/summer-cooling.json`.
- Current static product component: `site/src/site/components/ProductCard.astro`.
- Project design authority: `shipglowz_data/technical/design-system-authority.md`.
...
- `site/src/pages/guides/kitchen-gadgets.astro` and `site/src/pages/guides/summer-cooling.astro`: should become thin route wrappers around the shared guide template.
- `site/src/site/components/ProductCard.astro`: card data contract expands; existing fields must stay backward-compatible or current data must be updated in the same change.
- `site/src/site/data/*.json`: copy and schema-like conventions change; unsupported claims must be removed.
- `site/public/data/kitchen-gadgets.json`: may be stale duplicate guide data; implementation must decide whether to update, remove, or document its role.
- `site/public/llms.txt`: currently states "honest reviews" and "verified seller on Temu"; this may need correction if it overclaims review depth or verified seller status.
...
- Update editorial governance or content map only if the project decides to formalize guide surfaces under root `shipglowz_data/editorial/`; current `shipglowz_data/editorial/content-map.md` appears copied from another project and should not be treated as authoritative for Temu without cleanup.
- README updates are not required unless the public guide workflow becomes a developer-facing command or policy.
- Do not edit `shipglowz_data/workflow/TASKS.md` or `shipglowz_data/workflow/AUDIT_LOG.md` from this spec.
...
- `site/public/data` contains a stale copy that contradicts `site/src/site/data`.

## Implementation Tasks

- [ ] Task 1: Define the shared affiliate guide data contract.
  - Files: `site/src/site/data/kitchen-gadgets.json`, `site/src/site/data/summer-cooling.json`
...
- [ ] Task 2: Create a shared Astro guide template.
  - File: `site/src/site/components/AffiliateGuideTemplate.astro`
...
- [ ] Task 3: Upgrade static product cards for affiliate trust.
  - File: `site/src/site/components/ProductCard.astro`
...
- [ ] Task 4: Refactor existing guide routes to use the shared template.
  - Files: `site/src/pages/guides/kitchen-gadgets.astro`, `site/src/pages/guides/summer-cooling.astro`
...
- [ ] Task 5: Remove unsupported public claims.
  - Files: `site/src/site/data/*.json`, `site/public/data/kitchen-gadgets.json`, `site/public/llms.txt` if needed
...
- [ ] AC9: Given `site/public/llms.txt` or `site/public/data` contains contradictory public claims, when implementation finishes, then those claims are corrected, updated, or explicitly documented as out of scope with no active contradiction in shipped pages.
...
2. Run `python3 /home/claude/shipglowz/tools/design_system_drift_check.py --changed --format markdown`.
...
- Medium maintenance risk: `site/public/data` may be stale duplicate content. Mitigation: implementation must reconcile or document its role.
...
- Read first: `site/src/pages/guides/kitchen-gadgets.astro`, `site/src/pages/guides/summer-cooling.astro`, `site/src/site/components/ProductCard.astro`, `site/src/site/data/kitchen-gadgets.json`, `site/src/site/data/summer-cooling.json`, `site/src/layouts/Layout.astro`, `shipglowz_data/technical/design-system-authority.md`.
...
- Treat `shipglowz_data/editorial/content-map.md` as suspect migration debt because it references `tubeflow-site`, not TemuGlowz.
- Public guide content is French; internal ShipFlow headings and metadata stay English.
- Validation commands before handoff: `pnpm build:site`, `python3 /home/claude/shipglowz/tools/design_system_drift_check.py --changed --format markdown`, `python3 /home/claude/shipglowz/tools/shipflow_metadata_lint.py shipglowz_data/workflow/specs/temu-affiliate-guide-template-upgrade.md shipglowz_data/technical/design-system-authority.md`, plus the focused `rg` scans listed in `Test Contract`.
- Stop conditions for `102-sf-start`: stop and return to spec if the implementation needs a new site design authority beyond the current app-shell document, if a guide field requires raw HTML rendering to preserve intended content, if the shared template would break static rendering or force per-card hydration, or if `site/public/data` ownership cannot be resolved without widening scope.
