...
evidence:
  ...
  - "Current code shows mixed patterns: inline text errors, silent early returns, thrown store errors without page-level handling, and one-off dialog logic in app/src/ui/temu-shell/App.vue."
next_step: "/102-sg-start Unified in-app feedback and notifications"
---
...
- Integration into `app/src/ui/temu-shell/App.vue` so WebView-shell flows use the shared system.
- Integration into the highest-risk pages:
  ...
  - `app/src/app-pages/ListsPage.vue`
...
- The system must support both the shell route (`app/src/ui/temu-shell`) and the classic routed pages under `app/src/pages`.
...
- Existing shell CSS primitives in `app/src/ui/temu-shell/style.css` and app styles in `app/src/styles.css`.
...
2. Replace the one-off WebView product popup with the shared system.
   - Migrate `app/src/ui/temu-shell/App.vue` from custom popup state to shared notifications where possible.
...
- Added a shared Pinia notification store in `app/src/stores/notifications.ts`.
- Added a global notification host in `app/src/components/AppNotifications.vue`, mounted from `app/src/App.vue`.
- Added shared notification styling in `app/src/styles.css`.
- Replaced the one-off WebView invalid-product popup path with shared notifications in `app/src/ui/temu-shell/App.vue`.
- Migrated fallback WebView capture feedback in `app/src/ui/temu-shell/components/NetworkWebviewHost.vue`.
- Migrated critical recoverable flows in:
  ...
  - `app/src/app-pages/ProductDetailPage.vue`
  - `app/src/app-pages/ListsPage.vue`
- Added targeted notification-store tests in `app/src/stores/notifications.test.ts`.

Proof passed:

- `pnpm typecheck:full`
- `pnpm eslint` on the touched files
- `pnpm test:once app/src/stores/notifications.test.ts app/src/stores/shoppingSessions.test.ts app/src/utils/url.test.ts app/src/lib/importParser.test.ts`
