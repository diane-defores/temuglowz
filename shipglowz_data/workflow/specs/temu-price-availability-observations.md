...
depends_on:
  - artifact: "shipglowz_data/workflow/research/2026-06-11-temu-api-policy-price-availability.md"
    artifact_version: "1.0.0"
...
checklist_path: `shipglowz_data/workflow/test-checklists/temu-price-availability-observations.md`
...
- `shipglowz_data/workflow/test-checklists/temu-price-availability-observations.md`
...
- Existing product snapshot model in `app/src/types/domain.ts` and `app/src/stores/productSnapshots.ts`.
- Existing list model in `app/src/stores/shoppingLists.ts`.
- Existing WebView capture bridge in `app/src/lib/temuWebview.ts`, `app/src/ui/temu-shell/App.vue`, and `app/src-tauri/plugins/android-temu-webview/`.
- Existing cloud sync queue in `app/src/lib/cloudSync.ts` and related tests.
- Existing backup validation in `app/src/lib/backup.ts` and `app/src/lib/validators.ts`.
- Project docs in `shipglowz_data/technical/apps/temu-shopping-lists-android-app.md` and `shipglowz_data/technical/platforms/android.md`.
- Exploration report `shipglowz_data/workflow/research/2026-06-11-temu-api-policy-price-availability.md`.
...
- `shipglowz_data/technical/apps/temu-shopping-lists-android-app.md`: document observation model, local-first storage, sync posture, and policy guardrails.
- `shipglowz_data/technical/platforms/android.md`: document any Android notification/reminder behavior and WebView menu action.
- `shipglowz_data/workflow/test-checklists/temu-price-availability-observations.md`: create manual checklist.
...
1. Define observation domain types.
   - Target files: `app/src/types/domain.ts`, `app/src/lib/validators.ts`.
...
2. Add product observation store.
   - Target files: `app/src/stores/productObservations.ts`, `app/src/stores/productObservations.test.ts`.
...
3. Integrate observations with product snapshots.
   - Target files: `app/src/stores/productSnapshots.ts`, `app/src/stores/localPersistence.test.ts`, `app/src/stores/cloudSyncStoreIntegration.test.ts`.
...
4. Add manual observation UI.
   - Target files: `app/src/app-pages/ProductDetailPage.vue`, possibly new `app/src/components/ProductObservationForm.vue`.
...
5. Add explicit WebView observation action.
   - Target files: `app/src/ui/temu-shell/App.vue`, `app/src/lib/temuWebview.ts`, `app/src-tauri/plugins/android-temu-webview/android/src/main/java/com/temushoppinglists/temuwebview/TemuWebViewPlugin.kt`.
...
6. Add reminder preference.
   - Target files: `app/src/types/domain.ts`, `app/src/stores/productObservations.ts`, product detail UI.
...
7. Update backup/export and optional sync.
   - Target files: `app/src/lib/backup.ts`, `app/src/lib/validators.ts`, sync queue/domain types, related tests.
...
8. Add policy guardrails and docs.
   - Target files: `README.md`, `shipglowz_data/technical/apps/temu-shopping-lists-android-app.md`, `shipglowz_data/technical/platforms/android.md`, manual checklist.
...
- Read first: `app/src/types/domain.ts`, `app/src/stores/productSnapshots.ts`, `app/src/stores/shoppingLists.ts`, `app/src/lib/backup.ts`, `app/src/lib/validators.ts`, `app/src/lib/cloudSync.ts`, `app/src/ui/temu-shell/App.vue`, `app/src/lib/temuWebview.ts`, and `app/src-tauri/plugins/android-temu-webview/android/src/main/java/com/temushoppinglists/temuwebview/TemuWebViewPlugin.kt`.
...
- Do not edit `shipglowz_data/workflow/TASKS.md` in this skill run; `TASK-2026-06-10-013` remains the existing operational pointer.
