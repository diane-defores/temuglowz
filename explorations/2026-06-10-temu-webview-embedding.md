...
- `src-tauri/tauri.conf.json` - Confirms the current Android app is a Tauri app wrapping our own Vue/Vite frontend in a WebView.
- `src/lib/shareBridge.ts` - Confirms the current integration model is share/query payload import, not an embedded Temu browser.
- `.github/workflows/dev-builds.yml` - Confirms GitHub CI is intended to build a debug Android APK.
- `shipflow_data/workflow/specs/temu-shopping-lists-android-app.md` - Confirms the current spec treats direct Temu/cart capture as unresolved and higher risk.
